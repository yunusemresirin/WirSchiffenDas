...
class AnalysisWorkerTest {
    ...
    void reportsEveryEquipmentAndPreservesAttempt() {
        AnalysisCommand command = command(Map.of("coolingSystem", "STANDARD"));
        worker.execute(command);
        verify(management).reportStatus("analysis", ATTEMPT, "THERMAL", "RUNNING", "Analysis started");
        verify(management).reportResult("analysis", ATTEMPT, "THERMAL", "READY", "OK",
                "All equipment checks passed", Map.of("coolingSystem", "OK"));
        verify(next).startNext(command, "OK");
    }

    @Test
    void invalidEquipmentDoesNotSkipOtherEquipment() {
        var configuration = new HashMap<>(Map.of("coolingSystem", "STANDARD"));
        configuration.put("coolingSystem", "INVALID");
        worker.execute(command(configuration));
        verify(management).reportResult("analysis", ATTEMPT, "THERMAL", "FAILED", "FAILED",
                "Invalid or missing equipment: coolingSystem", Map.of("coolingSystem", "FAILED"));
    ...
    void missingEquipmentFailsWithCompleteEquipmentMap() {
        var configuration = new HashMap<>(Map.of("coolingSystem", "STANDARD"));
        configuration.remove("coolingSystem");
        worker.execute(command(configuration));
        verify(management).reportResult("analysis", ATTEMPT, "THERMAL", "FAILED", "FAILED",
                "Invalid or missing equipment: coolingSystem", Map.of("coolingSystem", "FAILED"));
    }
    ...
    void resultDeliveryFailureDoesNotContinueOrInventAContradictoryResult() {
        ...
        worker.execute(command(Map.of("coolingSystem", "STANDARD")));
