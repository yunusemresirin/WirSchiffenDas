...
class AnalysisManagementClientTest {
    private static final String URL = "http://management/internal/analyses/analysis/algorithms/THERMAL/status";
    ...
    private void send() {
        client.reportStatus("analysis", "attempt", "THERMAL", "RUNNING", "started");
    }
    ...
    void resultContainsAttemptAndEveryEquipmentResult() {
        server.expect(requestTo("http://management/internal/analyses/analysis/algorithms/THERMAL/result"))
                .andExpect(jsonPath("$.attemptId").value("attempt"))
                .andExpect(jsonPath("$.equipmentResults.coolingSystem").value("FAILED"))
                .andExpect(jsonPath("$.equipmentResults.length()").value(1))
                .andRespond(withNoContent());
        client.reportResult("analysis", "attempt", "THERMAL", "FAILED", "FAILED", "invalid equipment", Map.of("coolingSystem", "FAILED"));
        server.verify();
    }
}
