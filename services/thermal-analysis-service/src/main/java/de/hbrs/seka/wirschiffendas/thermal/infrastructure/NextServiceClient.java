...
public class NextServiceClient {
    ...
    private void fallback(AnalysisCommand command, String currentResult, Throwable throwable) {
        if (!definitelyNotAccepted(throwable)) {
            ...
            log.warn("Uncertain handoff to electrical-analysis-service for analysis {} attempt {}; management timeout handles recovery",
                    command.analysisId(), command.attemptId(), throwable);
            return;
        }
        managementClient.reportStatus(command.analysisId(), command.attemptId(), "ELECTRICAL", "FAILED",
                "electrical-analysis-service could not accept the analysis: " + throwable.getClass().getSimpleName());
    }
