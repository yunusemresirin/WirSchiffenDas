...
class AnalysisWorkerTest {
    ...
    void reportsEveryEquipmentAndPreservesAttempt() {
        AnalysisCommand command = command(Map.of("electricalSystem", "STANDARD"));
        worker.execute(command);
        verify(management).reportStatus("analysis", ATTEMPT, "ELECTRICAL", "RUNNING", "Analysis started");
        verify(management).reportResult("analysis", ATTEMPT, "ELECTRICAL", "READY", "OK",
                "All equipment checks passed", Map.of("electricalSystem", "OK"));
        verify(next).startNext(command, "OK");
    }

    @Test
    void invalidEquipmentDoesNotSkipOtherEquipment() {
        var configuration = new HashMap<>(Map.of("electricalSystem", "STANDARD"));
        configuration.put("electricalSystem", "INVALID");
        worker.execute(command(configuration));
        verify(management).reportResult("analysis", ATTEMPT, "ELECTRICAL", "FAILED", "FAILED",
                "Invalid or missing equipment: electricalSystem", Map.of("electricalSystem", "FAILED"));
    ...
    void missingEquipmentFailsWithCompleteEquipmentMap() {
        var configuration = new HashMap<>(Map.of("electricalSystem", "STANDARD"));
        configuration.remove("electricalSystem");
        worker.execute(command(configuration));
        verify(management).reportResult("analysis", ATTEMPT, "ELECTRICAL", "FAILED", "FAILED",
                "Invalid or missing equipment: electricalSystem", Map.of("electricalSystem", "FAILED"));
    }
    ...
    void resultDeliveryFailureDoesNotContinueOrInventAContradictoryResult() {
        ...
        worker.execute(command(Map.of("electricalSystem", "STANDARD")));
