...
class AnalysisManagementClientTest {
    private static final String URL = "http://management/internal/analyses/analysis/algorithms/ELECTRICAL/status";
    ...
    private void send() {
        client.reportStatus("analysis", "attempt", "ELECTRICAL", "RUNNING", "started");
    }
    ...
    void resultContainsAttemptAndEveryEquipmentResult() {
        server.expect(requestTo("http://management/internal/analyses/analysis/algorithms/ELECTRICAL/result"))
                .andExpect(jsonPath("$.attemptId").value("attempt"))
                .andExpect(jsonPath("$.equipmentResults.electricalSystem").value("FAILED"))
                .andExpect(jsonPath("$.equipmentResults.length()").value(1))
                .andRespond(withNoContent());
        client.reportResult("analysis", "attempt", "ELECTRICAL", "FAILED", "FAILED", "invalid equipment", Map.of("electricalSystem", "FAILED"));
        server.verify();
    }
}
