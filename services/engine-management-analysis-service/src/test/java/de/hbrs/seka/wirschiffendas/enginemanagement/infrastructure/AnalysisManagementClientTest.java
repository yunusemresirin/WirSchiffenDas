...
class AnalysisManagementClientTest {
    private static final String URL = "http://management/internal/analyses/analysis/algorithms/ENGINE_MANAGEMENT/status";
    ...
    private void send() {
        client.reportStatus("analysis", "attempt", "ENGINE_MANAGEMENT", "RUNNING", "started");
    }
    ...
    void resultContainsAttemptAndEveryEquipmentResult() {
        server.expect(requestTo("http://management/internal/analyses/analysis/algorithms/ENGINE_MANAGEMENT/result"))
                .andExpect(jsonPath("$.attemptId").value("attempt"))
                .andExpect(jsonPath("$.equipmentResults.engineManagementSystem").value("FAILED"))
                .andExpect(jsonPath("$.equipmentResults.length()").value(1))
                .andRespond(withNoContent());
        client.reportResult("analysis", "attempt", "ENGINE_MANAGEMENT", "FAILED", "FAILED", "invalid equipment", Map.of("engineManagementSystem", "FAILED"));
        server.verify();
    }
}
