...
Die Zerlegung folgt dem Grundsatz \emph{business capability first} \cite{evans2003ddd}. Services werden nicht nach Presentation-, Business- und Data-Layer getrennt, sondern entlang fachlicher Verantwortlichkeiten. Damit wird insbesondere das Anti-Pattern \emph{Wrong Cut} vermieden.
...
% Ports
\node[port] (pcfg) at (cfg.east) {};
\node[port] (pam) at (am.west) {};

% Port numbers
\node[plabel,left=1mm of cfg.west] {:8081};
\node[plabel,right=1mm of am.east] {:8082};

% Connections
\draw[-{Latex},thick] (cfg) -- (cfgdb);
\draw[-{Latex},thick] (am) -- (amdb);
\draw[-{Latex},dashed] (pcfg) -- node[above]{DTO / REST} (pam);
...
\node[plabel,right=2mm of ui.east] {Host :3000 $\rightarrow$ :80};
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{../diagrams/Bausteinsicht.png}
  \caption{Bausteinsicht: Frontend, zustandsbehaftete Services und Analyse-Services.}
\end{figure}
\vfill
...
\begin{tikzpicture}[
  node distance=10mm,
  every node/.style={font=\small},
  stepbox/.style={draw=darkblue,rounded corners,fill=lightblue,minimum width=18mm,minimum height=11mm,align=center}
]
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{../diagrams/Laufzeitsicht.jpeg}
  \caption{Laufzeitsicht: sequenzielle Choreographie der Analyse-Services.}
\end{figure}
\vfill
