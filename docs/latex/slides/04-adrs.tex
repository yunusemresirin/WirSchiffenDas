...
% --- ADR-04 -----------------------------------------------------------------
\begin{frame}[fragile]{ADR-04 -- Circuit Breaker mit Resilience4j}
  \adrbadge{ADR-04}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.54\textwidth}
      \begin{lstlisting}[style=java]
public void start(AlgorithmName algorithm,
    ...
    CircuitBreaker cb = circuitBreakers.get(algorithm);
    ...
    \begin{column}{0.42\textwidth}
      ...
      \begin{itemize}
        ...
        \item Analysis Management nutzt je Zielservice einen eigenen Breaker
          (Fluid, Thermal, Electrical, Engine Management).
        \item \code{nextService} schützt die jeweilige Choreographie-Kante
          zum nächsten Analyse-Service.
      \end{itemize}
      \begin{block}{Detail}
...
\begin{frame}{ADR-05 -- Container pro Microservice}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.5\textwidth}
      ...
      \textbf{Nachweis}
      \begin{itemize}
        \item 6 Backend-Dockerfiles + Frontend-Dockerfile.
        \item \code{docker-compose.yml} mit 7 Containern: Backend 8081--8086, Frontend 3000:80.
      \end{itemize}
    \end{column}
    \begin{column}{0.46\textwidth}
      \vspace*{-0.75cm}
...
% --- ADR-06 -----------------------------------------------------------------
\begin{frame}[fragile]{ADR-06 -- Keine Shared Persistence}
  \adrbadge{ADR-06}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.5\textwidth}
